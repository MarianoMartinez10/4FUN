...
display.flip = function(){
	...
	drawString(208, 352, 'COPAS');
	drawString(208, 400, 'NIVEL');
	drawString(208, 448, 'META');
	...
	drawString(blockSize, blockSize, 'PROX');
	drawString(blockSize*7, blockSize, '');
	...
	drawString((display.width-5)*blockSize, blockSize, 'PROX');
	drawString((display.width-11)*blockSize, blockSize, '');
	...
	if(!paused){
		...
		drawString(blockSize*3, blockSize*17, 'PAUSA');
		drawString(blockSize*23, blockSize*17, 'PAUSA');
	}
	if(p1.ai)drawString(blockSize*4, blockSize*10, 'DEMO');
	if((p1.ai || !p1.playing) && p1.blink && !paused){
		drawString(blockSize, blockSize*17, 'ESPACIO');
	}
	if(p2.ai)drawString(blockSize*24, blockSize*10, 'DEMO');
	if((p2.ai || !p2.playing) && p2.blink && !paused){
		drawString(blockSize*21, blockSize*17, 'ENTER');
	}
	if((p1.dead || p1.idle) && !p1.ai)drawString(blockSize, blockSize*17, '¡PERDISTE!');
	if((p2.dead || p2.idle) && !p2.ai)drawString(blockSize*21, blockSize*17, '¡PERDISTE!');
